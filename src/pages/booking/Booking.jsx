...
const  Booking =({ onClose }) => {
    ...
    return (
      ...
      <div className="flex">
        {activity.activity_schedules.map((schedule) => (
          <div
            key={schedule.id}
            className={`border border-gray-500 p-1 mr-2 ${selectedSlots[activityId] === schedule.id ? 'bg-red-700 text-white' : ''}`}
 ...
 <div className="mb-4 ml-12 font-bold text-lg">Activity Book</div>

 {activities.map((activity) => (
  <div key={activity.activity_id} className="mb-4 ml-10 font-semibold flex items-center">
    <div className="">
      <div className='flex flex-col popup-container'>
        <label className="inline-block font-bold text-2xl">
